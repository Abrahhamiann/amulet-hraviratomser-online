...
test('every public design has one canonical event category', () => {
  assert.equal(PUBLIC_DESIGN_KEYS.length, 21);
  ...
  assert.equal(templateCategoryForDesign('army-ceremonial'), 'military');
  assert.equal(templateCategoryForDesign('army-camouflage'), 'military');
...
test('each supported invitation category uses its matching editor fields', () => {
  ...
  assert.equal(templateEditorTypeForCategory('military'), 'military');
});
