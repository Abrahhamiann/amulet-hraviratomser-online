...
export default function ForgotPasswordPage() {
  ...
  return <section className="auth-page password-reset-page"><div className="auth-panel password-reset-panel">
    ...
    {step === 'email' && <form className="auth-form" onSubmit={sendCode}><label><span>Email</span><input type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com" required /></label><button className="auth-submit" disabled={busy}><Mail size={18} />{busy ? t('authWait') : t('authSendResetCode')}</button></form>}
    {step === 'code' && <form className="verification-form" onSubmit={verifyCode}><div className="verification-code-row">{code.map((digit, index) => <input key={index} ref={(el) => { refs.current[index] = el; }} value={digit} inputMode="numeric" maxLength={1} onChange={(e) => { const value = e.target.value.replace(/\D/g, '').slice(-1); setCode((current) => current.map((item, i) => i === index ? value : item)); if (value && index < 5) refs.current[index + 1]?.focus(); }} />)}</div><button className="auth-submit" disabled={busy}><ShieldCheck size={18} />{busy ? t('authChecking') : t('authConfirm')}</button></form>}
    {step === 'password' && <form className="auth-form" onSubmit={savePassword}><label><span>{t('authNewPassword')}</span><input type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} /></label><label><span>{t('authRepeatPassword')}</span><input type="password" autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required minLength={8} /></label><PasswordRequirements checks={checks} t={t} /><button className="auth-submit" disabled={busy}><KeyRound size={18} />{busy ? t('authWait') : t('authSavePassword')}</button></form>}
