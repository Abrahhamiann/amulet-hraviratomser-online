      ...
      <Card className="rounded-2xl border-border/60 shadow-[var(--shadow-soft)] overflow-hidden">
        <div className="flex flex-wrap items-center gap-3 p-4 border-b border-border/60 bg-secondary/30">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input value={q} onChange={(event) => setQ(event.target.value)} placeholder={`${t("invitations")}...`} className="pl-9 bg-background" />
          </div>
            ...
            <TableBody>
              {filtered.map((invitation: any) => (
                <TableRow key={invitation.id} className="border-border/60 hover:bg-secondary/30">
                  ...
                  <TableCell>{formatAdminCategory(invitation.category, lang)}</TableCell>
                  ...
                  <TableCell className="text-muted-foreground text-sm">{formatAdminDate(invitation.eventDate, lang)}</TableCell>
    ...
    <div className="grid gap-4 sm:grid-cols-2 py-2">
      ...
      {!form.orderId && (
        <>
          ...
          <div className="space-y-2"><Label>{t("date")}</Label><Input type="date" value={form.date} onChange={(event) => set("date", event.target.value)} /></div>
          <div className="space-y-2"><Label>{t("time")}</Label><Input value={form.time} onChange={(event) => set("time", event.target.value)} /></div>
          <div className="space-y-2 sm:col-span-2"><Label>{t("location")}</Label><Input value={form.location} onChange={(event) => set("location", event.target.value)} /></div>
          <div className="space-y-2 sm:col-span-2"><Label>{t("message")}</Label><Textarea value={form.message} onChange={(event) => set("message", event.target.value)} /></div>
        </>
      )}
    </div>
  );
