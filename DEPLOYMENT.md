...
API startup никогда не создаёт статические шаблоны и не удаляет активные шаблоны. Он только завершает удаление записей, уже явно помеченных администратором как удалённые. Каталог управляется через админ-панель. В production demo seed дополнительно заблокирован без явного аварийного флага.
